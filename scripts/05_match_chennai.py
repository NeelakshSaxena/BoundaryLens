...
def main():
    ...
    parcels_path = os.path.join("data", "processed", "chennai_synthetic_parcels.geojson")
    out_path = os.path.join("data", "processed", "chennai_buildings_linked_2d.geojson")

    if not os.path.exists(bldgs_path) or not os.path.exists(parcels_path):
        print(f"Error: Processed GeoJSON files are missing. Checked {bldgs_path} and {parcels_path}")
        return
